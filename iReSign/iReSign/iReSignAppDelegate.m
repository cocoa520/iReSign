...
{
    [self.window setTitle:@"汇美签名工具专版"];
    certsDic = [[NSMutableDictionary alloc] init];
    [flurry setAlphaValue:0.5];
    
    provisioningPathField.valueDelegate = self;
    defaults = [NSUserDefaults standardUserDefaults];
    
    // Look up available signing certificates
    [self getCerts];
    
    [bundleIDField setStringValue:@"com.xt.nc08"];
...
- (void)watchEntitlements:(NSFileHandle*)streamHandle {
    @autoreleasepool {
        ...
        NSRange range = [entitlementsResult rangeOfString:@"<?"];
        if (range.location != 0) {
            entitlementsResult = [entitlementsResult substringFromIndex:range.location];
        }
        NSLog(@"entitlementsResult:%@",entitlementsResult);
    }
}
...
- (void)doEntitlementsEdit
{
    @try {
        ...
        NSLog(@"doEntitlementsEdit exception:%@",exception);
    }
}
...
- (BOOL)isOSX1012Later {
    ...
    if (versionMajor >= 10 && versionMinor >= 12){
        return YES;
    }
    return NO;
}
...
- (void)signFile:(NSString*)filePath {
    ...
    NSString* cert = [certComboBox objectValue];
    if ([self isOSX1012Later]) {
        NSString* certId = [certsDic objectForKey:cert];
        if (certId.length > 0) {
            cert = certId;
        }
        //[certComboBox setStringValue:@"49896164EEFA3C841D38BF93B4D9C7FB577C0683"];
    }
    
    NSMutableArray *arguments = [NSMutableArray arrayWithObjects:@"-fs", cert, nil];
    ...
    @autoreleasepool {
        ...
        for (int i = 0; i <= [rawResult count] - 2; i+=2) {
            ...
            if (rawResult.count - 1 < i + 1) {
                ...
                // Valid object
                BOOL isOK = NO;
                NSString* certID = [rawResult objectAtIndex:i];
                @try {
                    NSRange start = [certID rangeOfString:@") "];
                    if (start.length > 0) {
                        NSInteger first = start.location + start.length;
                        certID = [certID substringFromIndex:first];
                        certID = [certID stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
                        isOK = [certID length] > 0;
                    }
                } @catch (NSException *exception) {
                    NSLog(@"watchGetCerts exception:%@",exception);
                }
                NSString* certIndetify = [rawResult objectAtIndex:i+1];
                [tempGetCertsResult addObject:certIndetify];
                if (isOK) {
                    if (![certsDic objectForKey:@"certIndetify"]) {
                        [certsDic setValue:certID forKey:certIndetify];
                    }
                }
            }
        }
...
- (void)textFieldValueDidChange:(IRTextFieldDrag*)sender {
    if (sender == provisioningPathField) {
        NSTask* task = [[NSTask alloc] init];
        [task setLaunchPath:@"/usr/bin/security"];
        [task setArguments:@[@"cms", @"-D", @"-i", provisioningPathField.stringValue]];
        ...
        NSFileHandle *handle = [pipe fileHandleForReading];
        NSString* result = [[NSString alloc] initWithData:[handle readDataToEndOfFile] encoding:NSUTF8StringEncoding];
        
        NSRange range = [result rangeOfString:@"<string>3589VJA289.c"];
        if (range.length > 0) {
            result = [result substringWithRange:NSMakeRange(range.location + range.length-1, 12)];
            NSLog(@"appid result:%@ bundleIDField:%@",result,bundleIDField.stringValue);
            if (![result isEqualToString:bundleIDField.stringValue]) {
                bundleIDField.stringValue = result;
            }
        }
    }
}
